...
$sql = $dbconnect->query("SELECT picture FROM sptest.user, sptest.news WHERE user.iduser = news.id_user and id_status = 5 and idnew ={$idnew} ");
...
function moderation($header, $date, $short_info, $picture, $full_info, $iduser, $comment, $idnew){
    ...
    global $dbconnect;
    if(isset($_POST['do_edit_publish'])) {
        //echo 'кнопка нажата';
        if(mb_strlen($_POST['header']) < 5 || mb_strlen($_POST['header']) > 300) {
            $error = 1;
            $_SESSION['message'] = [
    ...
    if(isset($_POST['do_edit_rough']))
    {
        ...
        if($_SESSION['message']['status'] != 'error')  {
            ...
            header('Location: archive.php?idnew='.$idnew);
        }
    }
    if(isset($_POST['do_edit_archive'])){

        $_SESSION['message'] = [
            'text' => 'Новость оставлена в архиве',
            'status' => 'success'
        ];
        header('Location: archive.php');
    }
