...
$sql = $dbconnect->query("SELECT picture FROM sptest.user, sptest.news WHERE user.iduser = news.id_user and id_status = 4 and idnew ={$idnew} ");
...
function moderation($header, $date, $short_info, $picture, $full_info, $iduser, $comment, $idnew){
    ...
    if(isset($_POST['do_edit_rough']))
    {
        ...
        if($_SESSION['message']['status'] != 'error')  {
            ...
            header('Location: publish.php?idnew='.$idnew);
        }
    }
