...
$sql = $dbconnect->query("SELECT dateEdit, header, full_info, short_info, news.date, idnew, picture, name FROM sptest.news WHERE id_user = {$id_user} and id_status = 4");
$i = 0;
...
if($_REQUEST['page']) {
    ...
    if ($_SESSION['logged_user']['role']== 'moderator'){
        $results = $dbconnect->query("SELECT * FROM sptest.user,  sptest.news where user.iduser = news.id_user  and id_status=4 ORDER BY idnew DESC LIMIT {$from},{$to}");
    }else {
        //модератор видит все новости (и свои, и других пользователей), остальные видят только свои
        $results = $dbconnect->query("SELECT * FROM sptest.user,  sptest.news where user.iduser = news.id_user and id_user={$id_user}   and id_status=4 ORDER BY idnew DESC LIMIT {$from},{$to}");
    }
}else {
    if ($_SESSION['logged_user']['role']== 'moderator'){
        $results =  $dbconnect->query("SELECT * FROM sptest.user, sptest.news where user.iduser = news.id_user and id_status=4 ORDER BY idnew DESC LIMIT 0,{$itemPerPage}");
    }
    else{
        $results =  $dbconnect->query("SELECT * FROM sptest.user, sptest.news where user.iduser = news.id_user and id_user={$id_user} and id_status=4 ORDER BY idnew DESC LIMIT 0,{$itemPerPage}");
    }
}
    ...
    <div class="container">
        <div class="row justify-content">
            <div class="col-md-10 col-md-offset-1 text-start">
                <?php if(isset($_REQUEST['idnew']) && $_SESSION['logged_user']['role'] == 'user'){
                    $id = (int)$_REQUEST['idnew'];
                    $_SESSION['news']['idnew'] = $id;
                   // $full = $dbconnect->query("SELECT dateEdit, header, full_info, short_info, date, idnew, picture, name FROM sptest.user, sptest.news WHERE user.iduser = news.id_user and id_status = 4 and idnew = '$id'");
                ...
                elseif (isset($_REQUEST['idnew']) && $_SESSION['logged_user']['role'] == 'moderator'){
                    $id = (int)$_REQUEST['idnew'];
                    $_SESSION['news']['idnew'] = $id;
                    $full = $dbconnect->query("SELECT dateEdit, header, full_info, short_info, date, idnew, picture FROM sptest.user, sptest.news WHERE user.iduser = news.id_user and id_status = 4 and idnew = '$id'");

                    $row = mysqli_fetch_assoc($full);

                    ?>
                    <h2>Редактирование новости №<?php echo $row['idnew']?></h2>
                    <form action="do_edit.php"  method="post" enctype="multipart/form-data">
                        ...
                        echo "<button class='btn btn-secondary' name='do_edit_archive' type='submit'>В архив</button>";
                        echo '</div>';
                        ?>
                    </form>
                    <script>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <?php while ($row = mysqli_fetch_assoc($results)){
                                ...
                                <tr>
                                    ...
                                    <?php
                                    echo '<td class="link-fullnews"><a href="/lk/publish.php?idnew='.$row['idnew'].'">Просмотреть</a></td>';
                                    $i++;
                                    ?>
                        ...
                        <?php
                        $count = $dbconnect->query("SELECT count(*) FROM  sptest.news where id_user = {$id_user} and id_status=4");
